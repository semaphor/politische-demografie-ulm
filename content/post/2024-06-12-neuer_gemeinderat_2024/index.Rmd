...
*Weitere Statistiken, die verwendeten Quellen und ein paar Erläuterungen und Hintergrundinformationen findest Du in* [unserer Analyse der Kandidat:innen vor der Wahl](/2024/05/30/vergleich-alter-bevoelkerung-wahllisten-gemeinderat/)*.*

Die Wahlergebnisse der Ulmer Gemeinderatswahl vom 9.6.2024 sind da und wir haben auf Basis der [vorläufigen amtlichen Ergebnisse](https://wahlergebnisse.komm.one/lb/produktion/wahltermin-20240609/08421000/praesentation/ergebnis.html?wahl_id=2496&stimmentyp=0&id=ebene_-8900_id_14330) eine erste Analyse erstellt. Abbildung \@ref(fig:plot-vgl-neuer-rat) stellt die Altersstruktur des neuen Gemeinderats (Mitte) im Vergleich mit der Ulmer Bevölkerung (oben) und des noch aktuellen Gemeinderats (Wahlperiode 2019-2024, unten) dar. Für die Vergleiche in diesem Artikel verschieben wir die Altersstruktur des aktuellen Gemeinderats auf den Zeitpunkt seiner Wahl 2019. Damals hatte dieser Rat ein mittleres Alter ([Median](/2024/05/30/vergleich-alter-bevoelkerung-wahllisten-gemeinderat#Bevölkerung%20und%20Kandidat:innen)) von `r median(aktRat$Alter) - 5` Jahren. **Der neue Gemeinderat ist mit einem mittleren Alter von `r median(ratNeu$Alter)` Jahren um `r abs(median(aktRat$Alter)-median(ratNeu$Alter) - 5)` Jahre älter als der aktuelle Rat bei seinem Start 2019.** Zum Vergleich: Die Ulmer Bevölkerung ist aktuell im Mittel `r demografieUlm2023Median` Jahre alt.
...
Wie man in der obigen Grafik erkennen kann hat sich durch die Wahl die Altersstruktur etwas verändert: Der **Altersschwerpunkt liegt beim neuen Rat im Bereich zwischen 60 und 66 Jahren**. In dieser relativ kleinen Altersgruppe mit einer Breite von nur 6 Jahren findet man ein Viertel der `r nrow(ratNeu)` Ratsmiglieder. Dieser Schwerpunkt hat sich im Vergleich mit dem letzten Rat um zwei bis drei Jahre nach oben verschoben. Auf der anderen Seite findet man: **Das jüngste Viertel des neuen Rates repräsentiert altersmäßig die jüngere Hälfte der Ulmer Bevölkerung** (`r count(ratNeu[ratNeu$Alter < median(demografieUlm2023Median), ])` Ratsmigleider sind jünger als `r demografieUlm2023Median`).

In der folgenden Tabelle (Abbildung \@ref(fig:quartile-neuer-rat)) findet man für den Vergleich die unterschiedlichen Gruppen in jeweils gleich große Viertel aufgeteilt. Neben dem mittleren Alter (Median, 50 %) sind auch die Grenzen zum unteren (25 %) und oberen Viertel (75 %) sowie das jüngste (0 %) und älteste (100 %) Alter in je einer Zeile angegeben.

```{r quartile-neuer-rat, fig.cap='Aufteilung in gleich große Viertel (sogenannte Quartile) nach Alter. Die Zeile 0 % enthält das jeweils jüngste Alter, die Zeile 100 % das älteste (Anmerkung: 90 Jahre bei "Ulm, ab 16" resultiert aus der Darstellung im vorliegenden Datensätzen, es gibt ältere Ulmer:innen). Die Zeilen 25, 50 und 75 % enthalten die Altersgrenzen zwischen den Vierteln. Die Spalte a enthält die Spannweiten der Viertel für "Ulm, ab 16", b die Spannweiten für "Neuer Gemeinderat" und c die Spannweiten für "Gemeinderat 2019-2024 (-5 Jahre)".', fig.align = 'center', echo=FALSE}
...
Abschließend (Abbildung \@ref(fig:altersVergleichKandidatinnenRat)) noch eine Gegenüberstellung der Altersverteilung der zur Wahl stehenden Kandidat:innen mit den nun gewählten Gemeinderät:innen.

```{r altersVergleichKandidatinnenRat, echo=FALSE, fig.cap='Vergleich Altersverteilung zur Wahl stehende Kandidat_innen (oben) und am 9.6.2024 gewählter Gemeinderat (unten). Jeweils Wahlalter (orange), Altersmedian (blau durchgehend) und Alters-Median Ulmer Bevölkerung (blau gestrichelt)', fig.width=10, fig.asp=0.8, message=FALSE, warning=FALSE, results='hide', fig.keep='all', fig.align = 'center', out.extra='style="display: block;"', echo=FALSE}
...
plotRatNeuGlatt <- plotRatNeuGlatt +
  #theme_ipsum() +
  theme(
    #axis.title.x = element_blank(),
    axis.text.x = element_blank(),
    plot.margin = unit(c(-50,0,0,0), 'points') #unit(c(top, right, bottom, left), units)
  )
