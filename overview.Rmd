...
for (i in seq_along(demografieUlm$Personen)) {
  ...
  demografieUlmDistr <- c(demografieUlmDistr, rep(mittleresAlter ,round(demografieUlm$Personen[i]/200)))
}
demografieUlmDistr <- as.data.frame(demografieUlmDistr)
demografieUlmDistr <- cbind(NA, NA, NA, 'Bevölkerung Ulm', 'Bevölkerung Ulm', demografieUlmDistr)
names(demografieUlmDistr) <- c('Beruf', 'Geburtsjahr', 'Stadtteil', 'Liste', 'Partei', 'Alter')

listenLabels <- rbind(data,demografieUlmDistr) %>% group_by(Liste) %>%
    summarise(n = n()) %>%
    mutate(text = paste0(Liste, ' (n=', n, ')'))

plotAgeListen <- ggplot(rbind(data,demografieUlmDistr), aes(x = Alter, y = Liste, fill = after_stat(x))) +
...
Die vollständige Liste, die alle angegebenen Berufe enthält, finden Sie in dieser Datei auf [GitHub](https://github.com/semaphor/politische-demografie-ulm/blob/main/daten/kandidat_innen-2024-09-06.csv).
