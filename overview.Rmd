...
# KANDIDATENLISTE:
#TODO/Frage Jakob Was ist eigentlich mit der ersten Spalte in der CSV, die nochmal einfach hochzählt?
data <- read.csv('daten/kandidat_innen-2024-09-06.csv')
#TODO Jakob 'Partei' durch 'Liste' ersetzen:
...
demografieUlmZehner <- demografieUlm %>% select(any_of(c("AltersgruppeZehner", "Personen"))) %>% group_by(AltersgruppeZehner) %>% summarise(across(everything(), sum))
...
plotAgeListen <- ggplot(data, aes(x = Alter, y = Partei, fill = after_stat(x))) +
...
plotAgeUlm <- ggplot(demografieUlmZehner, aes(x = AltersgruppeZehner, y = Personen)) + 
  geom_point() +
  geom_smooth() +
  ylim(0,NA)
