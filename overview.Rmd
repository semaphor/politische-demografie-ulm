...
Die folgende Tabelle erlaubt abschließend noch einen kurzen Blick auf die von den Kandidat*innen angegebenen Berufe.
Sie ist nach der Häufigkeit der Anzahl der Berufe sortiert.
...
Nach unserer ersten redaktionellen Bereinigung (siehe Code oben im Abschnitt Überblick) erhalten wir `r length(unique(listen$Beruf))` verschiedene Angaben zu Ausbildung, Beruf bzw. Tätigkeit, die in der folgenden Tabelle zusammengefasst werden.
Diese Bereinigung kann je nach Ausreizen des Interpretationsspielraums unterschiedlich ausfallen und ist ggf. noch unvollständig.
