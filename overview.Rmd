...
Die **mittlere Ulmerin ist somit `r demografieUlm2023Median` Jahre alt**.
...
Die **`r nrow(listen)` zur Wahl stehenden Kandidat_innen sind zwischen `r min(listen$Alter)` und `r max(listen$Alter)` Jahre** alt bzw. im Mittel `r max(listen$Alter)` Jahre.
In der folgenden Grafik ist im unteren Diagramm für jede Kandidat_in auf der x-Achse ein schwarzer Punkt eingetragen.
...
Also ist **der aktuelle Rat im Median um `r abs(demografieUlm2023Median - median(aktRat$Alter))` Jahre älter als die Ulmer:innen**.
...
Wäre dieser Rat nicht 2019, sondern in in diesem Jahr worden, müsste man 5 Jahre vom Alter abziehen. Damit ergibt sich ein theoretischer Median von `r median(aktRat$Alter) - 5` Jahren.
...
**Im Median sind nur zwei der sechzehn Listen jünger also die Ulmer Bevölkerung**:
...
Um ein runderes Bild zu bekommen, sollte man neben den Mittlwerten, auch die Altersverteilung betrachten. ^[
...
In der folgenden Grafik sind die Altersverteilungen aller Listen dargestellt, aufsteigend nach Median sortiert und im Verlgeich zur Ulmer Bevölkerung (oben).
