...
Damit ist die mittlere Ulmerin bspw. `r demografieUlm2023Median` Jahre alt.
...
Alle `r nrow(listen)` Kandidat_innen, die sich zur Wahl stellen sind zwischen `r min(listen$Alter)` und `r max(listen$Alter)` Jahre alt.
In der folgenden Grafik ist im untere Diagramm für jede Kandidat_in auf der x-Achse ein schwarzer Punkt eingetragen.
Die Kurve darüber fasst die Verteilung des Alters annäherungsweise zusammen.
Hier und in allen weiteren Grafiken zeigen die farbigen Linien außerdem folgendes an:

* Blaue Linien stellen die Medianwerte dar.
* Die gestrichelte blaue Linie liegt auf dem Medianwert der Ulmer Bevölkerung (`r demografieUlm2023Median` Jahre).
* Die etwas dickeren blauen Linien zeigen immer den Medianwert für die jeweilige Kurve/Gruppe an.
* Die orangen Linie markiert das Wahlalter 16.

TODO Plot

Im Median sind die Kandidat_innen `r abs(demografieUlm2023Median - median(listen$Alter))` Jahre älter als die Ulmer Wohnbevölkerung.

Die 41 Mitglieder des aktuellen Gemeinderat (Wahlperiode 2019-2024) sind zwischen `r min(aktRat$Alter)` und `r max(aktRat$Alter)` Jahre alt, der Alters-Median liegt bei `r demografieUlm2023Median`.
Im Median ist der aktuelle Rat also um `r abs(demografieUlm2023Median - median(aktRat$Alter))` Jahre älter als die Ulmer:innen.
Die Altersverteilung stellt die folgende Grafik dar:

TODO Plot

Würde dieser Rat in diesem Jahr gewählt werden, zieht man also 5 Jahre vom Alter ab, ergibt sich ein Median von `r median(aktRat$Alter) - 5` Jahren.
Damit wäre der Rat immer noch `r abs(demografieUlm2023Median - median(aktRat$Alter)) - 5` Jahre älter als die Ulmer Bevölkerung.

Zwischenfazit:
* Rat aktuell 22 bzw. 17 Jahre älter als Bevölkerung.
* Zur Wahl stehende Kandidat_innen sind im Median etwas junger, nämlich nur 11 Jahre Älter als die Ulmer:innen.
* Interssant wäre noch ein Vergleich der Alterststruktur der Kandidat:innen, die sich zu Wahl aufstellen, mit den dann gewählten Vertreter:innen. Leider liegen uns aktuell die Wahlvorschläge von 2019 nicht inklusive Angabe des Geburtsjahrs vor.

Welches Bild ergibt sich nun, wenn man auf die Alterststruktur der  `r length(levels(listen$Liste))` Listen schaut?
Im Median sind nur zwei Listen jünger also die Ulmer Bevölkerung:
...
Die 14 anderen Listen sind sozusagen älter als die Ulmer:innen: die Median-Werte liegen zwischen `r median(listen$Alter[listen$Liste == "BLO"])` und `r median(listen$Alter[listen$Liste == "UfA"])` Jahren (relativ zur Bevölkerung also zwischen `r abs(demografieUlm2023Median - median(listen$Alter[listen$Liste == "BLO"]))` und `r abs(demografieUlm2023Median - median(listen$Alter[listen$Liste == "UfA"]))`.
...
Um ein runderes Bild zu bekommen, sollte man neben Mittlwerten, aber auch die Altersverteilung betrachten.
Sotiert man die Listen per Median mit den jüngsten zuerst nach Alter und nimmt aus dieser Liste die erste fünf Plätze und macht man das dann umgekehrt mit den ältesten Listen zuerst, sieht man das FIXME (GRÜNE) und Ulm für Alle (UfA) beides mal in diesen ersten fünf Plätzen enthalten sind.
Schaut man wiederum auf den Median-Wert sieht man dass GRÜNE mit `r median(listen$Alter[listen$Liste == "GRÜNE"])` Jahren (relativ `r abs(demografieUlm2023Median - median(listen$Alter[listen$Liste == "GRÜNE"]))` Jahre älter) sehr "mittig" sind, UfA aber die "älteste" Liste im Feld stellt.
Beide haben also eine sehr weite Bandbreit im Alter ihrer Kandidat:innen, aber jeweils einen ganz unterschiedlichen Mittelwert.
An diesem Beispiel erkannt man wie einfach es ist mit "Statsitik zu lügen" (wie das Sprichwort sagt), indem man ganz bestimmte Werte auswählt und wiedergibt.
Deshalb finden sich in der folgenden Grafik eine vergleichende Darstellung der Altersverteilung aller Listen Liste aufsteigend nach Median sortiert und im Verlgeich zur Ulmer Bevölkerung (oben).
