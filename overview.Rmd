...
TODO Ausformulieren: Ulmer Bevölkerung zum Vergleich, mit Median; kein Durchschnittsalter Quantile; unten (evlt.? Details dazu).
...
plotAgeListenAlle <- listen %>%
  ...
  geom_density_ridges_gradient(
    ...
    #fill = "#E69F00"
    #aes(colour = factor(Liste))
    ) +
  #scale_colour_manual(values = c("#999999", "#E69F00", "#56B4E9", "#009E73", "#F0E442", "#0072B2", "#D55E00", "#CC79A7", "#999999", "#E69F00", "#56B4E9", "#009E73", "#F0E442", "#0072B2", "#D55E00", "#CC79A7", "#999999")) +
...
# PLOT ULMER BEVÖLKERUNG VS ALLE KANDIDAT_INNEN

#TODO Verbesserung: Quantile einzeichnen.

plotAlterBevoelkerungKandidatinnen <- listen %>%
  mutate(Liste = "Alle Kandidat_innen") %>%
  rbind(demografieUlm2023Distr %>%
          mutate(Liste = paste0('Ulm\n(n = ', length(demografieUlm2023Distr), ' )'))) %>%
  ggplot(aes(x = Alter, y = Liste)) +
  geom_density_ridges_gradient(
    scale = 45, 
    rel_min_height = 0.01, 
...
plotAlterBevoelkerungKandidatinnen

```

```{r fig.width=11, fig.height=5, echo=FALSE, message=FALSE, warning=FALSE, results='hide', fig.keep='all'}

# PLOT ULMER BEVÖLKERUNG VS ALLE KANDIDAT_INNEN, EINZELNE DIAGRAMME

plotOben <- ggarrange(NA, plotDemografieUlmDistr,
          labels = c("(A)", NA),
         #heights = c(12, 1.6),
         widths = c(0.11, 1),
         nrow = 1, ncol = 2)

plotUnten <- ggarrange(NA, plotAgeListenAlle,
                       labels = c("(B)", NA),
         #heights = c(12, 1.6),
         widths = c(0, 1),
         nrow = 1, ncol = 2)
...
```{r fig.width=11, fig.height=18, echo=FALSE, message=FALSE, warning=FALSE, results='hide', fig.keep='all'}
...
plotUnten <- ggarrange(NA, plotAgeListen,
                       labels = c("(B)", NA),
         #heights = c(12, 1.6),
         widths = c(0, 1),
         nrow = 1, ncol = 2)
...
```{r fig.width=11, fig.height=6, echo=FALSE, message=FALSE, warning=FALSE, results='hide', fig.keep='all'}
...
plotUnten <- ggarrange(NA, plotAktRat,
                       labels = c("(B)", NA),
         #heights = c(12, 1.6),
         widths = c(0, 1),
         nrow = 1, ncol = 2)

plotGanzUnten <- ggarrange(NA, plotAktRatMinus5,
                       labels = c("(C)", NA),
         #heights = c(12, 1.6),
         widths = c(0, 1),
         nrow = 1, ncol = 2)
...
suppressWarnings(
  ggarrange(plotOben, plotUnten, plotGanzUnten,
            vjust = 0,
            #widths = c(0.5, 1),
            nrow = 3, ncol = 1,
            heights = c(2,2,2)
...
# Spalte Tätigkeit erstmal mit Beruf füllen
listen$Tätigkeit <- listen$Beruf
