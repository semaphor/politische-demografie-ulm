...
Alternativ zur grafischen Darstellung kann man sich die Altersverteilungen auch noch zahlenmäßig detaillierter anschauen und dazu bspw. die unterschiedlichen Gruppen in jeweils gleich große Viertel aufteilen. Der Median teilt wie oben schon beschrieben die jeweilige Gruppe in dem Alter nach zwei gleich große Hälften auf. Er entspricht also der Altersgrenze für 50 % einer Gruppe. Zusammen mit dem schon genannten Medianwerten enthält die folgende Tabelle auch die Altersgrenzen für das untere (25 %) bzw. obere Viertel (75 %) der drei betrachten Gruppen. Die Werte in der Zeile 0% entsprechen dem jüngsten Alter in der jeweiligen. Gruppe, die Werte in der Zeile 100 % der ältesten Altersangabe (das maximale Alter für "Ulm" steht nicht für die ältesten Ulmer:innen, es ist im Datensatz als ältestes Alter enthalten, weil der Ulmer Statistik als älteste Gruppe "90 und älter" enthält).

```{r}
cbind(quantile(demografieUlm2023Distr$Alter), quantile(listen$Alter), quantile(aktRat$Alter))[1:5,] %>%
  kable(
    style = 'bootstrap',
...
Schaut man sich die obigen Kurven der Altersverteilungen an erkennt man direkt, wo bei der Ulmer Bevölkerung, den Kandidat:innen und dem aktuellen Gemeinderat die altersmäßigen Schwerpunkte liegen und wie stark verschiedene Altersgruppen ungefähr vertreten sind. Betrachtet man die Zahlenwerte in der Tabelle, erkennt man stärksten Viertel an den von den Altersgrenzen her kleinsten Gruppen: Das in der Ulmer Bevölkerung am stärksten vertretene Viertel ist zwischen 24 und 40 Jahre alt, bei den Kandidat:innen ist dieses Viertel zwischen 51 und 61 Jahre alt und beim aktuellen Gemeinderat zwischen 61 und 69.
...
Alternativ stellt das Statistische Bundesamt (DESTATIS) in der GENESIS-Datenbank für Ulm [@destatis:12411-0018_ulm] folgende Altersdaten mit aktuell letztem Stand vom 31.12.2022 zur Verfügung. Diese Daten sind also etwas älter und vor allem weniger fein aufgelöst. Alle Auswertungen in diesem Bericht basieren auf den Daten der Ulmer Statistik 2023, für Interessierte dennoch eine kurze Vorstellung:
